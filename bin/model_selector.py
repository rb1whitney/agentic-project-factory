...
def get_system_specs():
    ...
    try:
        output = subprocess.check_output(["nvidia-smi", "--query-gpu=memory.total,name", "--format=csv,noheader,nounits"], text=True)
...
def get_tier(ram, vram):
    if vram >= 5.5: return "gpu-6gb"
    if vram >= 3.5: return "gpu-4gb"
...
def get_recommendations(specs):
    ...
    if vram >= 5.5:
        ...
        if ram < 16: return ["qwen2.5-coder:1.5b", "gemma4:e4b"]
...
def main():
    ...
    if args.command == "check":
        ...
        if code != 0:
            subprocess.Popen(["su", "rb1whitney", "-c", f"OLLAMA_FLASH_ATTENTION='1' {ollama_bin} serve"], 
