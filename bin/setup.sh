...
if command_exists brew; then
  ...
  if [[ "$OS" == "macos" ]]; then
    ...
    run sh -c "echo 'eval \"\$(/opt/homebrew/bin/brew shellenv)\"' >> ~/.zprofile"
  else
    run eval "$(/home/linuxbrew/.linuxbrew/bin/brew shellenv)"
    run sh -c "echo 'eval \"\$(/home/linuxbrew/.linuxbrew/bin/brew shellenv)\"' >> ~/.bashrc"
...
if command_exists gcloud; then
  ...
  if [[ "$OS" == "macos" ]]; then
    ...
    run sh -c "curl https://sdk.cloud.google.com | bash -s -- --disable-prompts"
    run sh -c "source ~/.bashrc || true"
  fi
  ok "gcloud installed"
fi
...
if command_exists tfenv; then
  ...
  if [[ "$OS" == "macos" ]]; then
    ...
    run sh -c "echo 'export PATH=\"\$HOME/.tfenv/bin:\$PATH\"' >> ~/.bashrc"
...
run ansible-galaxy collection install amazon.aws --quiet
run ansible-galaxy collection install google.cloud --quiet
run ansible-galaxy collection install community.kubernetes --quiet
...
if command_exists python3; then
  run python3 "$REPO_ROOT/bin/ast-bridge/code_mapper.py" "$REPO_ROOT"
